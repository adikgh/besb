...
$(document).ready(function() {
	...
	$('.view_add').on('click', function() {
		if ($('.views_quantity').attr('data-sel') != 1) {
			...
			$.ajax({
				url: "/admin/products/item/view/get.php?view_add",
				type: "POST",
				dataType: "html",
				data: ({
	...
	$('.pitem_btn_delete').on('click', function() {
		btn = $(this)
		$.ajax({
			url: "/admin/products/item/view/get.php?pitem_delete",
			type: "POST",
			dataType: "html",
	...
	$('.pitem_qn').on('input', function () {
		...
		if (btn.parent().attr('data-quantity') == 0) mess('0');
			...
			$.ajax({
				url: "/admin/products/item/view/get.php?pitem_qn",
				type: "POST",
				dataType: "html",
				data: ({ 
					quantity: btn.html(),
	...
	$('.view_upd_pop').click(function(){
		...
		btn = $(this)
		$.ajax({
			url: "/admin/products/item/view/upd_q.php?pitem_d",
			type: "POST",
			dataType: "html",
			data: ({ id: btn.attr('data-id'), }),
			success: function(data){ 
	...
	$('html').on('click', '.view_upd', function() {
		if ($('.viewu_quantity').val().length >= $('.viewu_quantity').data('length')) {
			...
			$.ajax({
				url: "/admin/products/item/view/get.php?pitem_upd",
				type: "POST",
				dataType: "html",
				data: ({
