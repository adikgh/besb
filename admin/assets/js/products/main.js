...
$(document).ready(function() {
	...
	$('.product_search').on('input', function () {
		...
		if ($(this).val() == '') {
			...
			$.ajax({
				url: "/admin/products/search.php?product_search",
				type: "POST",
				dataType: "html",
				data: ({ result: $('.product_search').val(), }),
				success: function(data){
	...
	$('.product_item_search').on('input', function () {
		if ($(this).val() == '') {
			...
			$('.uc_u2qm').removeClass('dsp_n')
			$.ajax({
				url: "/admin/products/all/search_item.php?product_search",
				type: "POST",
				dataType: "html",
	...
	$('html').on('click', '.form_prd_online', function() {
		btn = $(this)
		$.ajax({
			url: "/admin/products/get.php?form_prd_online",
			type: "POST",
			dataType: "html",
			data: ({
				id: btn.attr('data-id'),
	...
	$('html').on('click', '.pr_btn_delete', function() {
		btn = $(this)
		$.ajax({
			url: "/admin/products/get.php?product_delete",
			type: "POST",
			dataType: "html",
	...
	$("html").on('change', '.product_img', function(){
		...
		if (window.FormData === undefined) mess('Бұл формат келмейді')
			...
			$.ajax({
				...
				url: "/admin/products/get.php?product_img_add",
				type: "POST",
				crossDomain: true,
				// method: "POST",
   ...
   $('.pr_img_add').click(function(){ $(this).siblings('.pr_img').click() })
	...
	$('html').on('click', '.product2_add_pop', function(){
		...
		btn = $(this)
		$.ajax({
			url: "/admin/products/pop_clone.php?clone",
			type: "POST",
			dataType: "html",
			data: ({ id: btn.attr('data-id'), }),
			success: function(data){ 
